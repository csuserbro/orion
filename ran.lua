...
function Library:Window(options)
    ...
    function tabsections:TabSection(options)
        -- ... (остальной код TabSection)
    end

    -- Добавление Settings Tab автоматически в конец
    function tabsections:AddSettingsTab()
        local settingsTab = tabsections:TabSection({ text = "Settings" })
        -- Можно добавить иконку если нужно
        return settingsTab
    end
        if options.text == "Settings" then
            local settingsTab = Instance.new("Frame")
            settingsTab.Name = "SettingsTab"
            settingsTab.Parent = allPages
            settingsTab.Visible = false
            settingsTab.BackgroundTransparency = 1
            settingsTab.Size = UDim2.new(0, 442, 0, 440)
            ...
            wmToggle.Text = config.watermark.enabled and "Watermark: Вкл" or "Watermark: Выкл"
            wmToggle.Size = UDim2.new(0, 200, 0, 24)
            wmToggle.Parent = settingsTab
            wmToggle.BackgroundColor3 = Color3.fromRGB(30,30,30)
            wmToggle.MouseButton1Click:Connect(function()
                config.watermark.enabled = not config.watermark.enabled
                wmToggle.Text = config.watermark.enabled and "Watermark: Вкл" or "Watermark: Выкл"
                UpdateWatermark()
            end)

            local wmDropdown = Instance.new("TextButton")
            local watermarkOptions = {"Название", "FPS", "Ping"}
            local watermarkState = 1
            wmDropdown.Text = "Watermark: "..watermarkOptions[watermarkState]
            wmDropdown.Size = UDim2.new(0, 200, 0, 24)
            wmDropdown.Parent = settingsTab
            wmDropdown.BackgroundColor3 = Color3.fromRGB(30,30,30)
            wmDropdown.MouseButton1Click:Connect(function()
                watermarkState = watermarkState + 1
                if watermarkState > #watermarkOptions then watermarkState = 1 end
                wmDropdown.Text = "Watermark: "..watermarkOptions[watermarkState]
                config.watermark.show_fps = (watermarkOptions[watermarkState] == "FPS")
                config.watermark.show_ping = (watermarkOptions[watermarkState] == "Ping")
                UpdateWatermark()
            end)

            local wmNameBox = Instance.new("TextBox")
            wmNameBox.Text = config.watermark.name
            wmNameBox.Size = UDim2.new(0, 200, 0, 24)
            wmNameBox.Parent = settingsTab
            wmNameBox.BackgroundColor3 = Color3.fromRGB(30,30,30)
            wmNameBox.FocusLost:Connect(function()
                config.watermark.name = wmNameBox.Text
                UpdateWatermark()
            end)

            -- Watermark UI
            local watermarkUI = Instance.new("TextLabel")
            watermarkUI.Name = "WatermarkUI"
            watermarkUI.Parent = game.CoreGui:FindFirstChild("Neverlose") or game.CoreGui
            watermarkUI.BackgroundTransparency = 1
            watermarkUI.Position = UDim2.new(0, 12, 0, 12)
            watermarkUI.Size = UDim2.new(0, 300, 0, 22)
            watermarkUI.TextColor3 = Color3.fromRGB(90, 200, 255)
            watermarkUI.Font = Enum.Font.GothamBold
            watermarkUI.TextSize = 17
            watermarkUI.Visible = config.watermark.enabled

            function UpdateWatermark()
                watermarkUI.Visible = config.watermark.enabled
                local txt = config.watermark.name
                if config.watermark.show_fps then
                    txt = txt.." | FPS: "..math.floor(workspace:GetRealPhysicsFPS())
                end
                if config.watermark.show_ping then
                    local stats = game:GetService("Stats"):FindFirstChild("Network")
                    local ping = stats and stats:FindFirstChild("Ping") and math.floor(stats.Ping:GetValue()) or "?"
                    txt = txt.." | Ping: "..ping
                end
                watermarkUI.Text = txt
            end
            UpdateWatermark()

            -- Blur
            local blurToggle = Instance.new("TextButton")
            blurToggle.Text = "Blur: Вкл/Выкл"
            blurToggle.Size = UDim2.new(0, 200, 0, 24)
            blurToggle.Parent = settingsTab
            blurToggle.BackgroundColor3 = Color3.fromRGB(30,30,30)

            -- Notifications
            local notifDropdown = Instance.new("TextButton")
            notifDropdown.Text = "Notifications: Позиция"
            notifDropdown.Size = UDim2.new(0, 200, 0, 24)
            notifDropdown.Parent = settingsTab
            notifDropdown.BackgroundColor3 = Color3.fromRGB(30,30,30)

            -- Hotkeys
            local hotkeyToggle = Instance.new("TextButton")
            hotkeyToggle.Text = "Hotkeys: Вкл/Выкл"
            hotkeyToggle.Size = UDim2.new(0, 200, 0, 24)
            hotkeyToggle.Parent = settingsTab
            hotkeyToggle.BackgroundColor3 = Color3.fromRGB(30,30,30)

            -- Цвет меню
            local colorBox = Instance.new("TextBox")
            colorBox.Text = "#1e1e1e"
            colorBox.Size = UDim2.new(0, 200, 0, 24)
            colorBox.Parent = settingsTab
            colorBox.BackgroundColor3 = Color3.fromRGB(30,30,30)

            -- Конфиги
            local saveBtn = Instance.new("TextButton")
            saveBtn.Text = "Сохранить конфиг"
            saveBtn.Size = UDim2.new(0, 200, 0, 24)
            saveBtn.Parent = settingsTab
            saveBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
            local loadBtn = Instance.new("TextButton")
            loadBtn.Text = "Загрузить конфиг"
            loadBtn.Size = UDim2.new(0, 200, 0, 24)
            loadBtn.Parent = settingsTab
            loadBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
            local removeBtn = Instance.new("TextButton")
            removeBtn.Text = "Удалить конфиг"
            removeBtn.Size = UDim2.new(0, 200, 0, 24)
        ...
        local config = Library._config
            ...
            local function SaveConfig()
                -- Здесь будет логика сохранения (например, в DataStore или локально)
            end
            ...
            -- Blur при открытии меню
            local blurEffect = nil
            local function ApplyBlur(state)
                if state then
            ...
            blurToggle.MouseButton1Click:Connect(function()
                config.blur.enabled = not config.blur.enabled
                ApplyBlur(config.blur.enabled)
            end)
            -- При открытии меню тоже применять blur если включено
            ApplyBlur(config.blur.enabled)

            -- Notifications dropdown
            local notifPositions = {"bottomleft", "topleft", "topright", "bottomright", "center"}
            notifDropdown.MouseButton1Click:Connect(function()
                local idx = table.find(notifPositions, config.notifications.position) or 1
                idx = idx + 1
                if idx > #notifPositions then idx = 1 end
                config.notifications.position = notifPositions[idx]
                notifDropdown.Text = "Notifications: "..config.notifications.position
            end)
            notifDropdown.Text = "Notifications: "..config.notifications.position
        ...
        function tabs:Tab(options)
            ...
            function sections:Section(options)
                ...
                function elements:Toggle(options)
                    -- Keybind Gear + Hotkey Visuals
                    local keybindFrame = Instance.new("Frame")
                    keybindFrame.Name = "keybindFrame"
                    keybindFrame.Parent = sectionFrame
                    keybindFrame.BackgroundTransparency = 1
                    keybindFrame.Size = UDim2.new(0, 220, 0, 22)

                    local gearBtn = Instance.new("ImageButton")
                    gearBtn.Name = "gearBtn"
                    gearBtn.Parent = keybindFrame
                    gearBtn.Size = UDim2.new(0, 18, 0, 18)
                    gearBtn.Position = UDim2.new(0, 0, 0, 2)
                    gearBtn.BackgroundTransparency = 1
                    gearBtn.Image = "rbxassetid://6031094678" -- иконка шестерёнки

                    local showHotkey = Instance.new("TextButton")
                    showHotkey.Name = "showHotkey"
                    showHotkey.Parent = keybindFrame
                    showHotkey.Text = config.hotkeys.enabled and "Hotkey: On" or "Hotkey: Off"
                    showHotkey.Size = UDim2.new(0, 60, 0, 18)
                    showHotkey.Position = UDim2.new(0, 22, 0, 2)
                    showHotkey.BackgroundColor3 = config.hotkeys.enabled and Color3.fromRGB(60, 180, 60) or Color3.fromRGB(60,60,60)

                    showHotkey.MouseButton1Click:Connect(function()
                        config.hotkeys.enabled = not config.hotkeys.enabled
                        showHotkey.Text = config.hotkeys.enabled and "Hotkey: On" or "Hotkey: Off"
                        showHotkey.BackgroundColor3 = config.hotkeys.enabled and Color3.fromRGB(60, 180, 60) or Color3.fromRGB(60,60,60)
                    end)
                    ...
                    hotkeyBtn.Parent = keybindFrame
                    hotkeyBtn.Text = options.key or "Key: None"
                    hotkeyBtn.Size = UDim2.new(0, 60, 0, 18)
                    hotkeyBtn.Position = UDim2.new(0, 86, 0, 2)
                    hotkeyBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
                    ...
                    input.InputBegan:Connect(function(key)
                        if waitingForKey and key.UserInputType == Enum.UserInputType.Keyboard then
                            options.key = key.KeyCode.Name
                            hotkeyBtn.Text = "Key: "..options.key
                            waitingForKey = false
                        end
                    end)

                    -- Визуализация активного бинда
                    local function UpdateHotkeyVisual(state)
                        if not config.hotkeys.enabled then
                            hotkeyBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
                        elseif state then
                            hotkeyBtn.BackgroundColor3 = Color3.fromRGB(80, 200, 80)
                        else
                            hotkeyBtn.BackgroundColor3 = Color3.fromRGB(60,60,60)
                        end
                    end

                    UpdateHotkeyVisual(options.state)

                    -- Обновлять при смене состояния
                    if options.callback then
                        local oldCallback = options.callback
                        options.callback = function(state)
                            UpdateHotkeyVisual(state)
                            oldCallback(state)
                        end
                    end

                    -- Остальной код тоггла ниже
    ...
    -- Автоматически добавить Settings Tab как последний
    tabsections:TabSection({ text = "Settings" })
    local win = {}
    win.TabSection = function(self, opts) return tabsections:TabSection(opts) end
    win.Tab = function(self, opts) return tabsections:TabSection(opts) end -- если Tab реализовано как TabSection
    return win
end
