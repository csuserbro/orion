...
do
    ...
    --
    local image_cache = {}
    local last_image_request = 0
    local min_image_request_delay = 0.5 -- секунды между http-запросами
    function utility:LoadImage(instance, imageName, imageLink)
        local data
        -- Кэширование изображений
        if image_cache[imageName] then
            data = image_cache[imageName]
        elseif isfile(library.folders.assets.."/"..imageName..".png") then
            data = readfile(library.folders.assets.."/"..imageName..".png")
            image_cache[imageName] = data
        else
            if imageLink then
                local now = tick()
                local wait_time = last_image_request + min_image_request_delay - now
                if wait_time > 0 then
                    wait(wait_time)
                end
                last_image_request = tick()
